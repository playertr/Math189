"""
Start file for hw4pr2 part(a) of Big Data Summer 2018
...
def grad_logreg(X, y, W, reg=0.0):
	...
	"""
	# TODONT: Find the gradient of logistic regression with respect to W
	"*** YOUR CODE HERE ***"
...
def NLL(X, y, W, reg=0.0):
	...
	"""
	# TODONT: Find the negative log likelihood of logistic regression
	"*** YOUR CODE HERE ***"
	#first try: incorrect

	# nll_of_x_i = np.multiply(y, np.log( sigmoid(X @ W))) \
	# 	+ np.multiply( (1 - y),  np.log( 1 - sigmoid(X @ W))) \
	# 	+  reg / 2.0 * np.linalg.norm(W, 2) ** 2
...
def grad_descent(X, y, reg=0.0, lr=1e-4, eps=1e-6, max_iter=500, print_freq=20):
	...
	# TODONT: run gradient descent algorithms
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		#calculate the (negative log) likelihood of getting the results
		# y with the data X given the weights W (and using reg to keep 
		# the entries of W small to avoid overfitting)
		nll= NLL(X, y, W, reg=reg)
		if np.isnan(nll):
			break
		nll_list.append(nll)

		#find which "direction" to go in W-space
		#by calculating gradient and updating W
		#to "slow down" descent, use lr < 1
...
def newton_step(X, y, W, reg=0.0):
	...
	"""
	# TODONT: Find the change of the weight according Newton's methods
	"*** YOUR CODE HERE ***"

	#instead of a diagonal, I use np.multiply, which is MUCH faster and also simpler.
	grad = grad_logreg(X, y, W, reg)
	mymu = np.multiply( sigmoid( X @ W ), (1 - sigmoid( X @ W )) )
	H = X.T @ ( np.multiply( mymu, X)) + reg * np.eye(X.shape[1])
...
def newton_method(X, y, reg=0.0, eps=1e-6, max_iter=20, print_freq=5):
	...
	while iter_num < max_iter and np.linalg.norm(step) > eps:
		...
		#find which "direction" to go in W-space
		#by calculating Newton-scaled gradient and updating W
		#to "slow down" descent, use lr < 1
...
def get_description(X, y, W):
	...
	"""
	# TODONT: Find the accuracy, precision, recall, and f-1 score of the prediction
	"*** YOUR CODE HERE ***"

	#copied from solution because it's not worth my time to write conditional logic
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	# TODONT: Find the list of accuracy, precision, recall, and f-1 score of the
