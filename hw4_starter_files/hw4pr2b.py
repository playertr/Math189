...
def NLL(X, y, W, reg=0.0):
	"""	This function takes in four arguments:
			...
			2) y, the label of the data with dimension m x k (one-hot)
			3) W, a weight matrix
	...
	"*** YOUR CODE HERE ***"
	mu = X @ W  #an m x k matrix (m observations, k labels)
	# where each row of the matrix corresponds to an input 
	# and each column corresponds to one of the labels

	exp_mu = np.exp( mu ) #element-wise exponentiation of mu

	# forms probabilities by normalizing each row of matrix
	# so that they sum to 1. Still m x k.
	prob = exp_mu / exp_mu.sum(axis=1).reshape(-1, 1)
	
	# multiply by m x k one-hot matrix element-wise
	# so that each row only keeps one value
	groundTruth = y * np.log(prob)

	#sum along the rows, then along the columns to return 
	#the log-likelihood of getting the set of results with 
	#the given input and weights. Add regularization factor.
...
def grad_softmax(X, y, W, reg=0.0):
	...
	"""
	# TODONT: Find the gradient of softmax regression with respect to W
	"*** YOUR CODE HERE ***"
	mu = X @ W  #an m x k matrix (m observations, k labels)
	# where each row of the matrix corresponds to an input 
	# and each column corresponds to one of the labels
	...
	# so that they sum to 1. Still m x k.
	prob = exp_mu / exp_mu.sum(axis=1).reshape(-1, 1)
	# the example solution .pdf calls prob "mu".
...
def predict(X, W):
	"""	This function takes in two arguments:
		...
		HINT:
			1) Firstly obtain the probability matrix according to the softmax
			   equation
