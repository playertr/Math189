...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		#find nearest label for each point in X
		for i in range(m):
			point = X[i, :]
			#subtractsevery row of clusters from point and finds norm
			distances = np.linalg.norm( point - clusters, axis=1 )
			nearestCluster = np.argsort(distances).item(0)
			label[i] = nearestCluster
		
		#update centers
		for clusterNum in range(k):
			#find points in X that were nearest to this cluster center
			ind = np.where(label == clusterNum)[0]
			if len(ind) > 0:
				#if there were points associated with this cluster, then
				#move this cluster to the mean center of them.
				clusters[clusterNum, :] = X[ind].mean(axis=0)
