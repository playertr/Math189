...
def nmf_cost(X, W, H):
	...
	#make sparse version of X
	sX = X.tocoo()

	#compute Fubini distance
	for i in range(len(sX.data)):
		row = sX.row[i]
		col = sX.col[i]
		data = sX.data[i]
		#clever way of only computing nonzero values
		#my original code was too slow so I borrowed this 
		#from the solution
...
def updateH(X, W, H):
	"""	This function takes in three arguments:
		...
		And returns a better estimate of H according to Lee and Seoung's
		multiplicative method that results in a nonincreasing Fubini distance 
		between the X and its approximation WH.
	"""
...
def updateW(X, W, H):
	"""	This function takes in three arguments:
		...
		And returns a better estimate of W according to Lee and Seoung's
		multiplicative method that results in a nonincreasing Fubini distance 
		between the X and its approximation WH.
	"""
	newW = W * ( (X @ H.T)/(W @ H @ H.T))
